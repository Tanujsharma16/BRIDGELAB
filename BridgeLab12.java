...
public class HotelManagementSystem {
    ...
    public static void main(String[] args) {
         ...
         List<Guest> guests = Arrays.asList(
                ...
                List<Booking> bookings = Arrays.asList(
                    ...
                    new Booking(1, 201, 1, LocalDate.now().minusMonths(1), LocalDate.now().minusMonths(1).plusDays(3)),
                    new Booking(2, 201, 2, LocalDate.now().minusMonths(2), LocalDate.now().minusMonths(2).plusDays(5)),
                    new Booking(3, 201, 3, LocalDate.now().minusMonths(3), LocalDate.now().minusMonths(3).plusDays(2)),
                    
                    
                    new Booking(4, 101, 4, LocalDate.now().minusMonths(4), LocalDate.now().minusMonths(4).plusDays(7)),
                    new Booking(5, 102, 1, LocalDate.now().minusMonths(5), LocalDate.now().minusMonths(5).plusDays(4)),
                    
                    
                    new Booking(6, 202, 2, LocalDate.now().minusMonths(7), LocalDate.now().minusMonths(7).plusDays(2)),
                    new Booking(7, 301, 3, LocalDate.now().minusMonths(8), LocalDate.now().minusMonths(8).plusDays(1)),
                    new Booking(8, 101, 1, LocalDate.now().minusMonths(13), LocalDate.now().minusMonths(13).plusDays(10))
                );
    ...
    public static void showTopGuests(List<Booking> bookings, List<Guest> guests) {
        ...
        bookings.stream()
                ...
                .limit(3)
                .forEach(entry -> {
                    String guestName = guestMap.getOrDefault(entry.getKey(), "Unknown");
                    System.out.printf("  - %s (ID: %d) with %d stays.%n", guestName, entry.getKey(), entry.getValue());
                });
    }
}